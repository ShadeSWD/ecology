/* Общие утилиты живых расчётов: форматирование чисел, строка
 * «формула = подстановка = результат», простые SVG-диаграммы.
 * Экспортируется одним объектом window.EC. */
'use strict';
window.EC = (function () {
  const AX = '#6b6b74', INK = '#16161a', ACC = '#155e75', RED = '#b3382e', GRN = '#1a7f37';
  ...
  /* строка без результата — только пояснение */
  function note(text) { return `<div class="calc">${text}</div>`; }
  ...
  /* подсветка изменившихся ячеек сводки */
  function flash(el) {
    if (!el) return;
    el.classList.add('upd');
    setTimeout(() => el.classList.remove('upd'), 60);
  }

  return { fmt, fmtE, fmtA, sup, row, note, esc, num, barsH, drawBars, flash,
    colors: { AX, INK, ACC, RED, GRN } };
})();
